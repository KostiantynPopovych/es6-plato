...
const GitHistory = require('./models/GitHistory');
const Churns = require('./models/Churns');
...
function inspect(files, outputDir, options, done) {
	...
	if ('projectRootPath' in options) {
		flags.churn = true;
		var commonBasePath = util.findCommonBase(files);
		var gitHistory = GitHistory.build({ directory: options.projectRootPath });
		var churns = Churns.from(gitHistory.history, { directory: options.projectRootPath });
	}
	...
	function runReports(files, done) {
		files.forEach(function reportEachFile(file) {
			...
			if (options.recurse && fs.statSync(file).isDirectory()) {
				...
				var churn = churns && churns.churnByPath.get(filePath);
				...
				if ('eslintStatsPath' in options) {
					var stats = require(options.eslintStatsPath);
					var fileReport = stats[path.resolve(filePath)];
					if (fileReport) {
						report.eslint = fileReport;
					} else {
						report.eslint = { messages: [] };
					}
				}
				reports.push(report);
				if (fileOutputDir) {
